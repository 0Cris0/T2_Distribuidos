...
def votar_por_lider(nodos, candidato):
    activos = {k: v for k, v in nodos.items() if v.get('activo')}
    #print(activos, "===========")
    aceptan = 1
    rechazan = 0
...
def eleccion_lider(nodos: dict, lider: dict):
    ...
    for nodo in nodos:
        ...
        if(contador_activos >= len(nodos)// 2 +1):
            ...
            break
    if(contador_activos < len(nodos)// 2 +1):
        #print(" >> [WARNING]: No hay suficientes nodos para mayoría")
        return (lider, nodos)
    ...
    while(lider == None and mayoria == True):
        for nodo in nodos.keys():
            nodo = nodos[nodo]
            if(nodo["activo"] == True):
                #print(f" >> >Nodo {nodo["name"]}, ({nodo["t_actual"]}+1/{nodo["timeout"]})")
                nodo["t_actual"] +=1
...
# funcion_raft("casos_Raft/test_03.txt")